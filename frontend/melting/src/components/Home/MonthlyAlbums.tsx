...
export default function MonthlyAlbum() {
  const [monthlyAlbums, setMonthlyAlbums] = useState<AlbumRankingResponseDto[]>(
    [],
  )
  ...
  useEffect(() => {
    const fetchSteadyAlbums = async () => {
      try {
        const response = await albumApi.getMonthlyAlbums()
        setMonthlyAlbums(response)
      } catch (err) {
        console.error(err)
  ...
  return (
    <>
      ...
      {loading || monthlyAlbums.length === 0 ? (
