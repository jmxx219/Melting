...
public interface AlbumRepository extends JpaRepository<Album, Long> {
	...
	@Query("SELECT a FROM Album a JOIN a.likesAlbums la WHERE la.member.id = :memberId AND a.isDeleted = false AND a.isPublic = true AND la.status = true ORDER BY a.createdAt DESC")
	Page<Album> findLikedAlbumsByMemberIdOrderByCreatedAtDesc(@Param("memberId") Long memberId, Pageable pageable);

	@Query("SELECT a FROM Album a JOIN a.likesAlbums la WHERE la.member.id = :memberId AND a.isDeleted = false AND a.isPublic = true AND la.status = true GROUP BY a.id ORDER BY COUNT(la) DESC")
